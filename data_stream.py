...
def run_spark_job(spark):
    ...
    # Create Spark configurations with max offset of 10/20/ or 200 per trigger
    # set up correct bootstrap server and port
    #for test option SparkSession property key/value pair (most optimal ??)
    df = spark \
        .readStream \
        .format('kafka') \
    ...
    # agg_df with groupby().count ?? to fix output in console ? 
    query = agg_df \
        .writeStream \
        .format('console') \
        .outputMode('complete') \
